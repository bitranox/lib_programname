...
function run_pytest() {
  ...
  if ! python3 -m pytest "${project_root_dir}" "$@" --cov=lib_programname; then
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : $HOME/venv/bin/lib_programname --version"
  if ! "$HOME/venv/bin/lib_programname" --version; then
